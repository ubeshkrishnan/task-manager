...
class AlertTile extends React.Component {
    state={
        dismissibleAlert:true
    }
  render(){
      const {dismissibleAlert} = this.state;
    return(
...
<p className="mb-0">Whenever you need to, be sure to use margin utilities to keep things nice and tidy.</p>
                ...
                <div className="bd-example mb-5">
                    <Alert variant='warning' show={dismissibleAlert} className="alert-dismissible"  >
                        ...
                        <button type="button" className="btn-close" data-dismiss="alert" aria-label="Close" onClick={() =>{ this.setState({ dismissibleAlert:false }) }}></button>
                    </Alert>
                    <SyntaxHighlighter language="html" style={a11yLight}  className="language-html">
